...
/// 给  内容视图`LsitView` 反馈显示或消失的回调
public protocol SwiftDropMenuControlContentAppearable {
    /// 即将显示下拉列表
    func willDisplay(forDropMenu menu: SwiftDropMenuControl)
    /// 已经显示下拉列表
    func didDisplay(forDropMenu menu: SwiftDropMenuControl)
    /// 即将隐藏下拉列表
    func willHidden(forDropMenu menu: SwiftDropMenuControl)
    /// 已经隐藏下拉列表
...
open class SwiftDropMenuControl: UIButton {
    ...
    open func show() {
        ...
        /// 计算菜单高度，需要有确定的`contentView` 确定高度，以撑起其父视图`masksView`
        //        contentView.contentViewHeight?.constant = listHeight
    ...
    private func willDisplay() {
        ...
        listView.willDisplay(forDropMenu: self)
    }
    /// 已经显示下拉列表
    private func didDisplay() {
        ...
        listView.didDisplay(forDropMenu: self)
    }
    /// 即将隐藏下拉列表
    private func willHidden() {
        if delegate?.responds(to: #selector(SwiftDropMenuListViewDelegate.willHidden(forDropMenu:))) == true {
            // 将要隐藏回调代理
            delegate?.willHidden?(forDropMenu: self)
        }
        listView.willHidden(forDropMenu: self)
    }
    /// 已经隐藏下拉列表
    private func didHidden() {
        ...
        listView.didHidden(forDropMenu: self)
    }
}
...
/// 适配SwiftDropMenuListView 的 `ListView`
extension SwiftDropMenuControlContentAppearable where Self: UIView {
    public func willDisplay(forDropMenu menu: SwiftDropMenuControl) {}
    public func didDisplay(forDropMenu menu: SwiftDropMenuControl) {}
    public func willHidden(forDropMenu menu: SwiftDropMenuControl) {}
    public func didHidden(forDropMenu menu: SwiftDropMenuControl) {}
}
extension UIView: SwiftDropMenuControlContentAppearable {}
